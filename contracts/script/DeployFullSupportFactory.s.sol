...
interface IHorizonCircle {
    function initialize(string memory, address[] memory, address) external;
}

contract FullSupportFactory {
    ...
    function createCircle(string memory name, address[] memory initialMembers) external returns (address) {
        ...
        IHorizonCircle(circleAddress).initialize(name, initialMembers, address(this));
...
contract DeployFullSupportFactory is Script {
    ...
    function run() external {
        ...
        console.log("=== Deploying Full Support Factory ===");
        console.log("Full Implementation:", FULL_IMPLEMENTATION);
        console.log("Registry:", REGISTRY);
        console.log("This implementation includes:");
        console.log("- Complete deposit/withdraw functionality");
        console.log("- requestCollateral for borrowing");
        console.log("- contributeToRequest for social collateral");
        console.log("- executeRequest with WETH->wstETH swap");
        console.log("- Morpho lending integration");
        console.log("- Loan repayment functionality");
        
        FullSupportFactory factory = new FullSupportFactory(
            FULL_IMPLEMENTATION,
            REGISTRY
        );
        
        console.log("\nFull Support Factory deployed:", address(factory));
        console.log("\nTo use this factory:");
        console.log("1. Update frontend FACTORY to:", address(factory));
        console.log("2. Update MIN_BLOCK_NUMBER to current block");
        console.log("3. Clear browser cache (IndexedDB)");
